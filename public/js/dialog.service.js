    ...
    return {
        ...
        execute: function() {
            ...
            DialogService = (function () {
                ...
                //this code was not using the Watson SDK to make the API calls, so replacing it
                DialogService.prototype.message = function (workspace_id, payloadToWatson) {
                    var headers = new http_1.Headers();
                    headers.append('Content-Type', 'application/json');
                    if (workspace_id) {
                        payloadToWatson = payloadToWatson || {};
//                        return this.http.post('/api/' + workspace_id.trim() + '/convmessage', JSON.stringify(payloadToWatson), { headers: headers }).map(function (res) { return res.json(); });
                        return this.http.post('/api/convmessage', JSON.stringify(payloadToWatson), { headers: headers }).map(function (res) { return res.json(); });
                    }
                    else {
